...
def extract_id_number_ocr(
    ...
    *,
    ...
    if hint == ID_TYPE_PRC:
        ordered = [(ID_TYPE_PRC, _PRC_FIND), (ID_TYPE_HKID, _HKID_FIND)]
    elif hint == ID_TYPE_HKID:
        ordered = [(ID_TYPE_HKID, _HKID_FIND), (ID_TYPE_PRC, _PRC_FIND)]
...
def enrich_number_from_ocr(
    *,
    ...
    if id_number and validate_id_number(id_type or ID_TYPE_PRC, id_number):
        return id_type, id_number
    if id_number and id_type == ID_TYPE_UNKNOWN:
        # 已有串但类型未知：尝试规范化
        for t in (ID_TYPE_PRC, ID_TYPE_HKID, ID_TYPE_PASSPORT):
            n = normalize_id_number(t, id_number)
            if validate_id_number(t, n):
                return t, n
