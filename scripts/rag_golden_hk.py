...
DURATION_MARKERS = ("审核周期", "3-4", "开户资料已正式提交")
...
def main() -> int:
    ...
    duration_hits = retriever.retrieve(DURATION_QUERY, scope=scope)

    if not duration_hits:

        print(f"FAIL  {DURATION_QUERY} → 无命中")

        failed += 1

    else:

        merged = "\n".join(h.text + h.step_title for h in duration_hits[:3])

        missing = [m for m in DURATION_MARKERS if m not in merged]

        top = duration_hits[0]

        if missing:

            print(f"FAIL  {DURATION_QUERY} → top-3 缺少: {missing}")
            ...
            print(
                ...
                f"OK    {DURATION_QUERY} → #1 step={top.step_title[:40]}"
