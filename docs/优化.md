...
现状分析
Q&A 系统现状
检索层 (src/rag/hybrid_retriever.py)：使用 RRF 融合 SQLite FTS5 关键词检索与 Qdrant 向量检索，有 query-specific boost（注意事项/时效查询）和 step-sibling 扩展。但缺少 MMR 多样性重排序，同一 source_path 或 step_id 的片段可能重复占据 top_k 位置。

重试机制 (src/agent/qa_agent.py + src/agent/query_rewriter.py)：重试循环中仅使用单一策略——query_rewriter.rewrite()（规则改写或 LLM 改写二选一）。检索范围 scope 在重试中不变，无 scope 放宽或关键词提取等备选策略。

忠实度检查 (src/agent/scoring/answer_scorer.py)：使用 2 字中文 bigram 重叠率（_faithfulness_heuristic）作为忠实度启发式，粒度粗，无法检测语义层面的幻觉。无 embedding 语义相似度辅助。

一致性控制：agent_runs 表存储历史 Q&A 记录，但无按语义相似度检索相似历史问题的功能。生成回答后不检查与历史回答的一致性。

回答长度：发送层有粗暴字节切分（_split_utf8_chunks），但 QA Agent 生成回答后直接返回全文，无内容感知的长度控制。

Prompt 格式化 (src/rag/prompt.py)：仅输出 来源: {source_path}\n{text}，RetrievedChunk 已有的 step_title、region、chunk_kind 等元数据未使用。

材料收集现状
文件去重 (src/wework/material_handler.py)：save_file_message() 每次上传直接落盘，无哈希去重。同一文件重复上传会重复存储和入库。

图片质量：文件校验后直接进入视觉识别，无清晰度预检。模糊图片浪费视觉 API 调用并返回 low_confidence。

字段标准化 (src/materials/form_parser.py)：仅对 id_type 和 id_number 做标准化，电话/邮箱/公司名无格式统一。不同来源（聊天文字 vs H5 表单 vs 视觉识别）格式可能不一致。

字段提取：extract_material_fields() 纯基于正则规则，用户自然语言描述不符合正则模式时可能提取不到字段。

跨字段校验 (src/materials/checklist.py)：progress_summary() 仅统计 received/missing，不校验字段间逻辑一致性（如证件类型与号码格式、电话区号与证件类型）。

缺失提醒 (src/wework/group_state_machine.py)：材料提醒仅在用户显式请求或提交材料后被动触发，COLLECTING 状态下发送业务 QA 消息时无主动提醒。
...
新增模块级函数 _bigram_jaccard(text_a: str, text_b: str) -> float：

对两个文本提取 2 字中文 bigram 集合，计算 Jaccard 相似度
空集返回 0.0
新增 HybridRetriever._mmr_rerank(self, candidates, top_k, lambda_) -> list[RetrievedChunk] 方法：
...
直到选满 top_k 或候选耗尽
修改 retrieve() 方法返回逻辑（当前第 257-264 行）：
...
在 QueryRewriter 类中新增 rewrite_with_strategy(self, question, hits, eval_result, strategy) -> tuple[str, str | None]：
...
修改 qa_agent.py 的 run() 方法重试循环（第 221-259 行）：
...
在 AnswerScorer.__init__ 中增加 embedder 延迟初始化（与 HybridRetriever 相同的懒加载模式）

新增 _embedding_faithfulness(self, answer, context) -> float 方法：

调用 Embedder.embed_texts([answer, context[:8000]])
计算余弦相似度
不可用时返回 -1.0
修改 score() 方法（第 65-69 行区域）：
...
在 ExternalGroupStore 中新增 get_recent_agent_runs(self, roomid, *, limit=20) -> list[dict]：查询最近 N 条已回复的 agent_runs 记录

在 QAAgent 中新增 _check_consistency(self, question, answer, roomid) -> tuple[bool, str]：

用 Embedder 计算当前问题与历史问题的余弦相似度
找到最相似的历史问题（超过阈值时）
用 LLM 判断两个回答是否矛盾
在 LLMClient 中新增 check_answer_contradiction(self, q1, a1, q2, a2) -> bool：通过 chat_json 让 LLM 判断矛盾

在 _run_knowledge_mode 返回前调用一致性检查（需将 roomid 参数传入 _run_knowledge_mode），矛盾时追加免责声明并记录 trace
...
在 LLMClient 中新增 summarize_answer(self, question, answer, max_chars) -> str：LLM 压缩回答到指定字数内

在 QAAgent 中新增 _enforce_response_length(self, question, answer) -> str：

超过 agent_response_max_bytes 时，先估算目标字符数（UTF-8 中文约 3 字节/字符）
启用摘要时调用 llm.summarize_answer
摘要仍超长或禁用摘要时，回退到 UTF-8 安全截断（在句号/换行处截断）
在 _run_knowledge_mode 和 _run_contextual_mode 返回前调用 _enforce_response_length

新增静态方法 _truncate_utf8(text, max_bytes) -> str：UTF-8 安全截断，在句号/换行处截断并追加提示
...
DB 层 - 在 _migrate_columns 中新增 file_hash 列到 group_materials 表，并创建索引 idx_group_materials_hash

DB 层 - 新增 find_material_by_hash(self, roomid, file_hash) -> dict | None 方法

DB 层 - 修改 upsert_material 签名，新增 file_hash: str = "" 参数

Handler 层 - 在 save_file_message 中 validate_upload 之后计算 SHA-256 哈希，调用 find_material_by_hash 检查重复，命中则返回 "duplicate" 跳过落盘

在 _persist_file 中传递 file_hash 到 upsert_material

在 notify_classification 中增加重复文件提示

新增常量 DUPLICATE_FILE = "duplicate"
...
新增模块级函数 _check_image_quality(data: bytes) -> tuple[bool, float]：
...
在 save_file_message 中，vision_enabled 为 True 且 materials_image_quality_enabled 时，调用 _check_image_quality，不通过则返回 "rejected_blur"

在 notify_classification 中增加模糊图片提示

新增常量 REJECTED_BLUR = "rejected_blur"

在 requirements.txt 中添加 opencv-python-headless
...
新增 _normalize_phone(raw: str) -> str：去空格/横线，识别香港号码（852 前缀补 +852）、大陆号码（1[3-9] 开头补 +86），保留已有 + 前缀

新增 _normalize_email(raw: str) -> str：去空格、转小写、去尾部多余标点

新增 _normalize_company_name(raw: str) -> str：统一全角/半角括号，压缩连续空格，去首尾标点

在 extract_material_fields 函数末尾（第 228 行 id_type/id_number 标准化之后）添加对 contact_email、applicant_email、contact_phone、applicant_phone、company_name_en、company_name_cn 的标准化调用
...
在 LLMClient 中新增 extract_material_fields_llm(self, text) -> dict[str, str]：通过 chat_json 让 LLM 从自然语言提取 14 个标准字段，过滤无效 key

在 extract_material_fields 函数末尾（标准化之后）添加 LLM 兜底逻辑：

若 materials_llm_extraction_enabled 且正则提取字段数 < materials_llm_extraction_min_fields 且文本长度 >= 10
调用 LLM 提取，仅补充正则未提取到的字段
LLM 不可用时静默回退
对 LLM 补充的字段也执行标准化
将标准化逻辑抽取为 _normalize_fields(fields: dict) -> dict，在正则提取后和 LLM 补充后统一调用
...
在 checklist.py 中新增 validate_cross_fields(materials: dict) -> list[dict[str, str]] 函数，包含 4 条规则：
...
在 progress_summary 返回值中增加 cross_field_issues 字段

在 group_state_machine.py 的材料入库步骤后调用 validate_cross_fields，error 级问题阻止进入 REVIEW 并发送校验提示

在 format_progress_text 中展示跨字段问题（warning 用提醒标记，error 用错误标记）
...
在 group_state_machine.py 的 GroupStateMachine 中新增：
...
在消息处理流程中调用 _maybe_proactive_reminder：

_execute_plan 方法末尾
QA 回复发送后
消息计数器为内存态，进程重启后重置（可接受——重启后重新计数不会导致刷屏）
...
实施顺序与依赖关系
建议实施顺序（按依赖关系排列）
优化 6（prompt 元数据丰富）- 无依赖，改动最小，仅修改 prompt.py
优化 9（字段格式标准化）- 无依赖，仅修改 form_parser.py
优化 1（MMR 重排序）- 仅改 hybrid_retriever.py
优化 7（文件去重）- DB 迁移 + handler 改动
优化 8（图片质量预检）- 需引入 opencv-python-headless 依赖
优化 3（Embedding 忠实度）- 改 answer_scorer.py
优化 2（多策略重试）- 改 query_rewriter.py + qa_agent.py
优化 5（长度控制）- 改 qa_agent.py + openai_client.py
优化 4（一致性检查）- 改 db.py + qa_agent.py + openai_client.py
优化 10（LLM 提取兜底）- 改 form_parser.py + openai_client.py
优化 11（跨字段校验）- 改 checklist.py + group_state_machine.py
优化 12（主动提醒）- 改 checklist.py + group_state_machine.py
同文件串行合并注意
...
假设与决策
MMR 相似度计算：使用 bigram Jaccard 而非 embedding 余弦相似度，因为 MMR 在检索热路径上执行，embedding 调用会增加延迟。bigram Jaccard 足以识别文本重复片段。

多策略重试 scope 放宽：relax_scope 将 scope 从 "hk" 扩大到 "all"，可能引入内地相关片段。这是预期行为——当香港域知识库无法命中时，扩大范围是合理的回退策略。

Embedding 忠实度：使用 Embedder 的 embed_texts 方法（复用现有 text-embedding-3-small 模型），不引入额外模型依赖。每次评分增加约 100-200ms 延迟，可通过配置关闭。

一致性检查 embedding 缓存：当前方案每次检查都重新 embed 历史 20 条问题。如性能成为问题，可后续优化为缓存历史问题的 embedding vector（本次不在范围内）。

图片质量预检降级：opencv-python-headless 不可用时安全降级（跳过检查），不影响主流程。

文件去重粒度：采用精确去重（SHA-256 整文件哈希），不做近似去重（MinHash+LSH）。材料收集场景中重复上传通常是完全相同的文件，精确去重已足够。

主动提醒额度控制：enforce_quota=True 确保遵守 KF 5/48h 额度限制。群聊场景不受额度限制但仍受速率限制。

LLM 提取兜底触发条件：正则提取字段数 < 2 且文本长度 >= 10 时触发，避免对短消息或已充分提取的消息浪费 LLM 调用。
...
c:\Users\EDY\.trae-cn\work\6a745790e090f6949ccfb902\qa_material_optimization_plan.md