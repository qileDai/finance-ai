...
STEP_DESCRIPTIONS = {
    ...
    "feishu": "① 飞书群对接客户，发送材料清单，监听 /资料 和 /start 指令",
    "collect": "② 搜集客户材料",
    "confirm": "② 和客户确认材料",
    "package": "③ 打包材料文件夹",
...
def cmd_feishu_bot(_args: argparse.Namespace) -> None:
    ...
    def cmd_docs(msg: FeishuMessage) -> None:
        ...
        client.send_group_text(
            msg.chat_id,
            f"以上是香港公司注册所需材料清单，请准备后发送 /start 开始注册流程。",
        )
    ...
    print(f"[飞书] 已注册指令: /资料, /start, /help")
...
def main() -> None:
    ...
    feishu_parser = sub.add_parser("feishu-bot", help="启动飞书群机器人（监听 /资料 /start 指令）")
